...
class _AreaReportPageState extends State<AreaReportPage> {
  ...
  Future<void> _askQuestion() async {
    ...
    try {
      ...
      setState(() {
        _qaHistory.add({"role": "agent", "text": answer});
      });
    } catch (e) {
      setState(() {
        _qaHistory.add({
          "role": "agent",
          "text": "Could not get answer: $e",
        });
      });
    } finally {
...
try {
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            "Searching reputable sources & connecting the dots",
            style: TextStyle(
              color: textSecondary,
              fontSize: 13,
            ),
  ...
  Widget _buildQaSection(ThemeData theme, Color textPrimary) {
    return Card(
                ...
                Expanded(
                  child: TextField(
                    ...
                    decoration: const InputDecoration(
                      hintText: "Ask about schools, crime, rental demand, growth, etc.",
                    ),
                  ),
                ),
                const SizedBox(width: 10),
                ElevatedButton(
                  onPressed: _qaLoading ? null : _askQuestion,
                  child: _qaLoading
                      ? const SizedBox(
                          width: 16,
                          height: 16,
  ...
  Widget _buildReport(
    ...
    final r = report ?? {};
final sections = (r["sections"] is List) ? r["sections"] as List : [];    
final keyInsights = (r["key_insights"] is List) ? r["key_insights"] as List : [];
final sources = (r["sources"] is List) ? r["sources"] as List : [];
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            (r["title"] ?? widget.areaInput).toString(),
            style: TextStyle(
              fontSize: 24,
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Icon(Icons.summarize_rounded, size: 15, color: theme.primaryColor),
                ...
                _buildNarrative((r["executive_summary"] ?? "").toString(), theme),
              ],
            ),
          ),
          const SizedBox(height: 20),
                ...
                child: Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Expanded(
                        child: _buildNarrative(e.value.toString(), theme),
                      ),
                    ],
                  ),
                ),
          ...
          _buildQaSection(theme, textPrimary),
  ...
  Widget build(BuildContext context) {
    ...
    final textSecondary =
        theme.textTheme.bodyMedium?.color?.withAlpha(180) ?? Colors.grey;
